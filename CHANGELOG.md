...
- **Heatmap voxel hero logo** — the hero section now features a draggable isometric eye-shaped voxel structure. Faces are solid-filled with a heatmap gradient (green at the inner shell, red at the outer edge) that shimmers as the structure rotates. Users can drag or use arrow keys to spin it; momentum carries the rotation after release and decays back to the auto-spin speed. Fully respects `prefers-reduced-motion` (no auto-spin; rAF halts when velocity settles).
