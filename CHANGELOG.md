...
- **Analysis Report section** — scrollable report below the interactive heatmap canvas. Presents findings narratively: a hero heatmap composite, a one-line primary finding headline derived from the rule-of-thirds breakdown, a first-fixation coordinate note, a 3×3 rule-of-thirds grid (max-attention cell highlighted), a duration comparison strip showing all three viewing durations side by side, and a methodology note with links to the reading guide and methodology docs. The report appears after primary inference and updates in-place as background durations complete. Near-tie copy detection: if two regions are within 10% of each other the headline uses "split between X and Y" rather than overclaiming a single dominant zone.
